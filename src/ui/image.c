...
void draw_ui_image(ui_image_t* image) {
    float scale_x = image->node.width / image->image.width;
    float scale_y = image->node.height / image->image.height;

    if(image->image.format == IMAGE_FORMAT_1BIT) {
        for(int y = 0; y < image->node.height; y++) {
            for(int x = 0; x < image->node.width; x++) {
                bool pixel = BIT_READ(image->image.data[(int)((int)(x / 8) * scale_x) * (int)(image->image.width / 8) + (int)(y * scale_y)], x % 8);
